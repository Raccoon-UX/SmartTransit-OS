  ...
  return (
    ...
    >
      <div className="relative z-10 max-w-sm w-full space-y-6 text-center">
        {/* Brand Icon & Name */}
        <div className="mx-auto w-14 h-14 rounded-xl bg-[#1769D1] flex items-center justify-center text-white font-bold shadow-subtle">
        ...
        <div className="space-y-2 pt-2">
          <div className="h-2 w-full rounded-full bg-[#E5E0D8] dark:bg-slate-800 overflow-hidden">
            <div
              className="h-full bg-[#1769D1] rounded-full transition-all duration-200"
              style={{ width: `${progress}%` }}
            />
          </div>
          <div className="text-right text-xs font-mono font-bold text-[#1769D1]">{progress}%</div>
        </div>

        {/* Warm Municipal Initialization Checklist */}
        <div className="p-4 rounded-xl bg-white dark:bg-navy-900 border border-[#E5E0D8] dark:border-slate-800 shadow-subtle text-left space-y-2.5 text-xs font-mono">
          {CHECKLIST_ITEMS.map((item, idx) => {
            ...
            return (
              <div key={idx} className="flex items-center space-x-2.5">
                {isDone ? (
                  <CheckCircle2 className="w-4 h-4 text-[#218A63] shrink-0" />
                ) : isCurrent ? (
                  <ArrowRight className="w-4 h-4 text-[#1769D1] animate-pulse shrink-0" />
                ) : (
