...
export function DispatchActivityTimeline({ activities = [], className = '' }) {
  return (
    <div className={cn('p-6 rounded-3xl bg-white dark:bg-navy-900 border border-slate-200 dark:border-slate-800 shadow-sm space-y-4 text-left', className)}>
      <div className="flex items-center justify-between pb-2 border-b border-slate-100 dark:border-slate-800">
        ...
        <span className="text-xs font-mono text-emerald-500 font-bold">Audit Event Logger</span>
      </div>
      ...
      <div className="space-y-3">
        {activities.map((act) => (
          <div
            key={act.id}
            className="p-3.5 rounded-2xl bg-slate-50 dark:bg-navy-850 border border-slate-100 dark:border-slate-800/80 flex items-start space-x-3 text-xs font-mono"
          >
...
export default DispatchActivityTimeline;
